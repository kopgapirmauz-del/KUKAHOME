    ...
    for raw_line in text.splitlines():
        ...
        if line.lower().startswith("model:"):
            ...
            if lang_key in LANG_KEYS and value:
                normalized = "zh" if lang_key == "cn" else lang_key
                info_map[normalized] = value
    return {"model": model, "type": product_type, "type2": product_subtype, "info": info_map}
...
def build_manifest() -> list[dict[str, object]]:
    ...
    for folder in folders:
        ...
        info = parse_info(info_path.read_text(encoding="utf-8"), fallback_model) if info_path.exists() else {"model": fallback_model, "type": "Mehmonxona", "type2": "", "info": "Premium collection"}
        images = sorted(
