...
function clientContactCellHtml(contact) {
  ...
  if (parsed.display === "-") return `<span class="cell-text cell-contact">-</span>`;
  ...
  if (!parsed.phoneDisplay) {
    if (linkHtml) return `<span class="client-contact-wrap">${linkHtml}</span>`;
    if (parsed.tgHref) {
      return `<span class="client-contact-wrap"><a class="client-contact-link raw-link" href="${escapeHtml(parsed.tgHref)}" target="_blank" rel="noopener noreferrer">${escapeHtml(parsed.display)}</a></span>`;
    }
    return `<span class="client-contact-wrap"><span class="cell-text cell-contact">${escapeHtml(parsed.display)}</span></span>`;
  }
  const menuItems = [
    parsed.telHref
      ? `<a class="client-contact-menu-item" href="${escapeHtml(parsed.telHref)}"><span class="client-contact-menu-icon" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M6.62 10.79a15.05 15.05 0 0 0 6.59 6.59l2.2-2.2a1 1 0 0 1 1-.24 11.36 11.36 0 0 0 3.56.57 1 1 0 0 1 1 1V20a1 1 0 0 1-1 1A17 17 0 0 1 3 4a1 1 0 0 1 1-1h3.49a1 1 0 0 1 1 1 11.36 11.36 0 0 0 .57 3.56 1 1 0 0 1-.24 1l-2.2 2.23Z"/></svg></span><span>${escapeHtml(t("contactCall"))}</span></a>`
      : "",
    `<button class="client-contact-menu-item" type="button" data-contact-copy="${escapeHtml(encodeURIComponent(parsed.copyText))}"><span class="client-contact-menu-icon" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M8 8a2 2 0 0 1 2-2h9v13a2 2 0 0 1-2 2h-7a2 2 0 0 1-2-2V8Zm-3 9H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1h-5a4 4 0 0 0-4 4v8Z"/></svg></span><span>${escapeHtml(t("contactCopy"))}</span></button>`,
    parsed.tgHref
      ? `<a class="client-contact-menu-item" href="${escapeHtml(parsed.tgHref)}" target="_blank" rel="noopener noreferrer"><span class="client-contact-menu-icon" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M21.5 4.4a1 1 0 0 0-1-.14L3.7 11.3a1 1 0 0 0 .07 1.87l4.22 1.47 1.5 4.67a1 1 0 0 0 1.77.3l2.42-3.15 4.2 3.08a1 1 0 0 0 1.57-.57l2.12-13.49a1 1 0 0 0-.07-.58ZM9.2 13.8l8.77-6.09-7.05 7.23-.26 2.42-.82-2.55-.64-1.01Z"/></svg></span><span>${escapeHtml(t("contactTelegram"))}</span></a>`
      : "",
  ].filter(Boolean).join("");
  return `<span class="client-contact-wrap has-menu"><button class="client-contact-main-btn" type="button" data-contact-menu-toggle>${escapeHtml(parsed.phoneDisplay)}</button><span class="client-contact-menu">${menuItems}</span>${linkHtml}</span>`;
}
