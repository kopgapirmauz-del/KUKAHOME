...
function renderNotifications() {
  ...
  refs.notifList.innerHTML = pageItems.map((n) => {
    ...
    return `<div class="notif-item ${unreadClass}"><div class="notif-row"><div>${escapeHtml(notificationText(n))}</div><span class="notif-actions">${readAction}${deleteAction}</span></div><div class="notif-time">${escapeHtml(fmtDateTime(n.createdAt || n.created_at))}</div></div>`;
  }).join("");
...
function getPrimaryPagesByRole(role) {
  if (role === "admin") return ["clients", "integrations", "warehouse", "sales", "warranty", "settings"];
  if (role === "manager") return ["clients", "warehouse", "sales", "warranty", "settings"];
  if (role === "hr") return ["clients", "hr", "warranty", "sales", "settings"];
  if (role === "cashier") return ["clients", "sales", "warranty", "settings"];
  if (role === "skladchi") return ["clients", "warehouse", "warranty", "sales", "settings"];
  return ["clients", "settings"];
}

function getAllowedPagesByRole(role) {
  const primary = getPrimaryPagesByRole(role);
  const all = ["clients", "integrations", "warehouse", "sales", "warranty", "priceLabel", "settings"];
  if (role === "admin" || role === "hr") all.push("hr");
  return [...new Set(primary.concat(all))];
}
