async function init() {
  ...
  startRemoteSync();
  if (/[?&](meta_oauth|meta_ads_oauth)=/.test(window.location.search) && state.user) {
    if (typeof switchPage === "function") switchPage("settings");
    if (typeof switchSettingsTab === "function") switchSettingsTab("channels");
    if (typeof initSettingsChannelsUI === "function") initSettingsChannelsUI();
  }
}

const LS_SIDEBAR_COLLAPSED = "crm_sidebar_collapsed";
const LS_SESSION_LAST_ACTIVE = "premium_crm_session_last_active_v1";
const LS_SESSION_LOGIN_AT = "premium_crm_session_login_at_v1";
const LS_LAST_PAGE_PREFIX = "premium_crm_last_page_v1";
const SESSION_IDLE_TIMEOUT_MS = 6 * 60 * 60 * 1000;
const SESSION_MAX_AGE_MS = 24 * 60 * 60 * 1000;

// ---------------------------------------------------------------------------
// API session token: stored only in sessionStorage (cleared when the tab/
// browser closes), never persisted alongside remembered credentials, and
// never written into state.db (so it can't end up in any local snapshot).
// ---------------------------------------------------------------------------
const LS_API_TOKEN = "premium_crm_api_token_v1";

function setApiToken(token) {
  if (token) sessionStorage.setItem(LS_API_TOKEN, token);
  else sessionStorage.removeItem(LS_API_TOKEN);
}

function getApiToken() {
  return sessionStorage.getItem(LS_API_TOKEN) || "";
}
...
function pageStorageKey(userId) {
  return `${LS_LAST_PAGE_PREFIX}_${String(userId || "")}`;
}

function saveLastPageForUser(page) {
  const userId = String(state.user?.id || "");
  if (!userId || !page) return;
  const key = pageStorageKey(userId);
  sessionStorage.setItem(key, page);
  localStorage.setItem(key, page);
}

function restoreLastPageForUser() {
  const userId = String(state.user?.id || "");
  if (!userId) return;
  const key = pageStorageKey(userId);
  const savedPage = sessionStorage.getItem(key) || localStorage.getItem(key);
  if (!savedPage) return;
  state.page = String(savedPage);
}
...
function clearSessionActivity() {
  sessionStorage.removeItem(LS_SESSION_LAST_ACTIVE);
  localStorage.removeItem(LS_SESSION_LAST_ACTIVE);
}
...
function clearSessionLogin() {
  sessionStorage.removeItem(LS_SESSION_LOGIN_AT);
  localStorage.removeItem(LS_SESSION_LOGIN_AT);
}
...
function bindSessionActivityTracker() {
  let lastMarkedAt = 0;
  const mark = () => {
    if (!state.user) return;
    const now = Date.now();
    if (now - lastMarkedAt < 30000) return;
    lastMarkedAt = now;
...
function bindEvents() {
  ...
  refs.menuToggle.addEventListener("click", () => {
    if (window.matchMedia("(max-width: 900px)").matches) {
      setSidebarOpen(!refs.sidebar.classList.contains("open"));
      return;
    }
    const collapsed = document.body.classList.contains("sidebar-collapsed");
    setSidebarCollapsed(!collapsed);
  });
  if (refs.sidebarCollapseBtn) {
    refs.sidebarCollapseBtn.addEventListener("click", () => {
      const collapsed = document.body.classList.contains("sidebar-collapsed");
      setSidebarCollapsed(!collapsed);
    });
  }
  ...
  if (refs.settingsChannelsTab) refs.settingsChannelsTab.addEventListener("click", () => switchSettingsTab("channels"));
...
function setSidebarCollapsed(collapsed, persist = true) {
  ...
  const next = desktop ? Boolean(collapsed) : false;
  document.body.classList.toggle("sidebar-collapsed", next);
  if (refs.sidebarCollapseBtn) {
    refs.sidebarCollapseBtn.classList.toggle("is-collapsed", next);
    const label = next ? "Menyuni yoyish" : "Menyuni yig'ish";
    refs.sidebarCollapseBtn.setAttribute("aria-label", label);
    refs.sidebarCollapseBtn.setAttribute("title", label);
  }
  if (persist) localStorage.setItem(LS_SIDEBAR_COLLAPSED, next ? "1" : "0");
}

function syncSidebarCollapseByViewport() {
  if (window.matchMedia("(max-width: 900px)").matches) {
...
function switchPage(page, keepPageIndex) {
  ...
  if (page === "clients") {
    renderFilters();
    renderTableWithLoading();
    if (REMOTE_DB_ENABLED && state.user) {
      loadClients().then((loaded) => {
  ...
  } else if (page === "priceLabel") {
    if (typeof renderPriceLabelPage === "function") renderPriceLabelPage();
  }
}
...
function getPrimaryPagesByRole(role) {
  ...
  if (role === "director") return ["clients", "pipeline", "integrations", "warehouse", "sales", "hr", "warranty", "priceLabel", "settings"];
