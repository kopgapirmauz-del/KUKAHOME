...
function renderIntegrationColumnsManager() {
  ...
  const rows = integrations.columns.map((col, idx) => {
    ...
    const actions = state.user?.role === "admin"
      ? `<div class="chip-actions">${locked ? "" : `<button class="action-btn" type="button" data-column-edit="${escapeHtml(id)}" aria-label="edit"><svg viewBox="0 0 24 24"><path d="m3 17.25 9.81-9.81 2.75 2.75L5.75 20H3v-2.75Zm14.71-8.04-2.92-2.92 1.42-1.42a1 1 0 0 1 1.42 0l1.5 1.5a1 1 0 0 1 0 1.42l-1.42 1.42Z"/></svg></button>`}</div>`
      : "";
...
function bindIntegrationEvents() {
  ...
  if (modal) {
    modal.querySelectorAll("button[data-platform-quick]").forEach((btn) => {
      btn.addEventListener("click", () => {
        const platform = String(btn.dataset.platformQuick || "instagram");
        if (form) form.platform.value = platform;
      });
    });
  }
