...
let integrationEditingColumnId = "";
...
function renderIntegrationSettingsPanel() {
  ...
  modal.querySelectorAll("button[data-platform-quick]").forEach((btn) => {
    const platform = String(btn.dataset.platformQuick || "instagram");
    const connected = integrations.connections.some((c) => c.platform === platform);
    btn.classList.toggle("connected", connected);
  });
  renderIntegrationConnections();
  renderIntegrationColumnsManager();
}

function showIntegrationConnectStep(step, platform = "instagram") {
  const quick = document.getElementById("integrationPlatformQuick");
  const form = document.getElementById("integrationConnectionForm");
  const startBtn = document.getElementById("integrationStartConnectBtn");
  if (step === "platforms") {
...
function resetIntegrationColumnForm() {
  integrationEditingColumnId = "";
  const form = document.getElementById("integrationColumnForm");
  if (!form) return;
  form.reset();
...
function renderIntegrationColumnsManager() {
  const wrap = document.getElementById("integrationColumnsManager");
  if (!wrap) return;
  const integrations = ensureIntegrationsState();
  const rows = integrations.columns.map((col, idx) => {
    const id = String(col.id || "");
    const color = integrationColumnAccent(id, idx);
    const locked = id === "new";
  ...
  wrap.querySelectorAll("button[data-column-edit]").forEach((btn) => {
    btn.addEventListener("click", () => {
      const integrations = ensureIntegrationsState();
      const row = integrations.columns.find((c) => String(c.id) === String(btn.dataset.columnEdit || ""));
      const form = document.getElementById("integrationColumnForm");
      if (!row || !form) return;
      integrationEditingColumnId = String(row.id);
      const titleInput = form.querySelector("input[name='title']");
      const colorInput = form.querySelector("input[name='color']");
      if (titleInput) titleInput.value = String(row.title || "");
      if (colorInput) colorInput.value = integrationColumnAccent(String(row.id || ""), 0);
    });
  });
  wrap.querySelectorAll("button[data-column-delete]").forEach((btn) => {
    btn.addEventListener("click", async () => {
      if (state.user?.role !== "admin") return;
      if (!(await confirmPermanentDelete())) return;
      const id = String(btn.dataset.columnDelete || "");
      const integrations = ensureIntegrationsState();
      integrations.columns = integrations.columns.filter((c) => String(c.id || "") !== id);
      integrations.leads.forEach((lead) => {
        if (String(lead.columnId || "") === id) lead.columnId = "new";
      });
      saveDB();
      renderIntegrationsPage();
      renderIntegrationSettingsPanel();
    });
  });
}
...
function bindIntegrationEvents() {
  ...
  const form = document.getElementById("integrationConnectionForm");
  const startConnectBtn = document.getElementById("integrationStartConnectBtn");
  const connectionBackBtn = document.getElementById("integrationConnectionBackBtn");
  const columnForm = document.getElementById("integrationColumnForm");
  const columnResetBtn = document.getElementById("integrationColumnResetBtn");
  const addColumnBtn = document.getElementById("integrationAddColumnBtn");
  ...
  if (settingsBtn && modal) settingsBtn.addEventListener("click", () => {
    renderIntegrationSettingsPanel();
    showIntegrationConnectStep("idle");
    toggleModal(modal, true);
  });
  if (startConnectBtn) startConnectBtn.addEventListener("click", () => showIntegrationConnectStep("platforms"));
  if (connectionBackBtn) connectionBackBtn.addEventListener("click", () => showIntegrationConnectStep("platforms"));
  if (modal) {
    modal.querySelectorAll("button[data-platform-quick]").forEach((btn) => {
      btn.addEventListener("click", () => {
        const platform = String(btn.dataset.platformQuick || "instagram");
        ...
        if (form) form.reset();
        showIntegrationConnectStep("form", platform);
      });
    });
  }
  ...
  if (columnForm) {
    columnForm.addEventListener("submit", (e) => {
      ...
      if (state.user?.role !== "admin") return;
      const integrations = ensureIntegrationsState();
      const fd = new FormData(columnForm);
      const title = String(fd.get("title") || "").trim();
      const colorRaw = String(fd.get("color") || "").trim();
      const color = /^#([0-9a-fA-F]{3}|[0-9a-fA-F]{6})$/.test(colorRaw) ? colorRaw : "#111111";
      if (!title) {
        showToast(t("fillRequired"));
        return;
      }
      if (integrationEditingColumnId) {
        const row = integrations.columns.find((c) => String(c.id) === String(integrationEditingColumnId));
        if (row && String(row.id) !== "new") {
          row.title = title;
          row.color = color;
        }
      } else {
        integrations.columns.push({ id: uid("col"), title, color });
      }
      saveDB();
      resetIntegrationColumnForm();
      renderIntegrationsPage();
  ...
  if (addColumnBtn) {
    addColumnBtn.addEventListener("click", () => {
      ...
      resetIntegrationColumnForm();
      const titleInput = columnForm?.querySelector("input[name='title']");
...
function renderIntegrationConnections() {
  ...
  list.querySelectorAll("button[data-conn-edit]").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      showIntegrationConnectStep("form", row.platform);
    });
  });
