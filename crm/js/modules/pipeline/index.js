(() => {
  ...
  async function exportPipelineExcel() {
    const rows = filteredItems();
    const headers = [t("number"), copy().stage, copy().quality, t("contact"), copy().nextAction, copy().lastContact, copy().value, copy().note, copy().lostReason];
    const body = rows.map((item, idx) => [
      idx + 1,
      stageLabel(item.stage),
      qualityLabel(item.temperature),
      item.client?.contact || "",
      item.nextActionAt ? fmtDate(item.nextActionAt) : "",
      item.lastContactAt ? fmtDate(item.lastContactAt) : "",
    ...
    await exportRowsToExcel({
      ...
      sheetName: "Pipeline",
      fileName: `pipeline_${state.lang}.xlsx`,
      headers,
      rows: body,
    });
  }
