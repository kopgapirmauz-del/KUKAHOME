...
async function onClientSubmit(e) {
  ...
  if (!state.editingClientId && typeof canCreateClientBase === "function" && !canCreateClientBase()) return;

  if (state.editingClientId) {
    const client = state.db.clients.find((x) => x.id === state.editingClientId);
    const previousManagerId = String(client?.managerId || "");
