...
function defaultOpeningRegulation(position) {
  const title = String(position || "").trim();
  const lower = title.toLowerCase();
  if (lower.includes("yuk") || lower.includes("gruz") || lower.includes("tashuv")) {
    return "Ishga taklif qilinadi\n\nYuk tashuvchi - o'rnatuvchi\n(Gruzchik-ustanovshik)\n\nTalablar:\n- Erkak kishi\n- Jismonan baquvvat\n- Mebel sohasida tajriba\n\nIsh vaqti:\n- 09:00 dan 18:00 gacha\n\nOylik maosh:\n- 4 000 000 - 5 000 000 so'm\n\nTaklif qilamiz:\n- Qulay ish joyi va barqaror ish\n- Doimiy ish va rivojlanish imkoniyati\n\nBatafsil ma'lumot uchun:\n📞 95 885 22 33";
  }
  return "";
}

function ensureOpeningRegulation(row) {
  const normalized = row && typeof row === "object" ? row : {};
  const current = String(normalized.regulation || normalized.note || normalized.details || "").trim();
  if (current && current !== "-" && current !== "--" && current !== "---") return current;
  return defaultOpeningRegulation(normalized.position || "") || "-";
}

function openingRegulationPreview(row) {
...
function bindHrEvents() {
  ...
  if (refs.hrCreateVacancyForm) {
    ...
    refs.hrCreateVacancyForm.addEventListener("focusin", (event) => {
      const target = event.target;
      if (!target) return;
      if (target.id === "hrVacancyPositionInput" || target.id === "hrVacancyRegulationInput") {
        hrVacancyEmojiTarget = target;
      }
    });
  }

  const insertHrEmoji = (emoji) => {
    const icon = String(emoji || "").trim();
    if (!icon || !refs.hrCreateVacancyForm) return;
    const fallback = document.getElementById("hrVacancyRegulationInput") || document.getElementById("hrVacancyPositionInput");
    const target = hrVacancyEmojiTarget || fallback;
    if (!target) return;
    const start = Number.isFinite(target.selectionStart) ? target.selectionStart : String(target.value || "").length;
    const end = Number.isFinite(target.selectionEnd) ? target.selectionEnd : String(target.value || "").length;
    const value = String(target.value || "");
    target.value = value.slice(0, start) + icon + value.slice(end);
    const nextPos = start + icon.length;
    if (typeof target.setSelectionRange === "function") target.setSelectionRange(nextPos, nextPos);
    target.focus();
    target.dispatchEvent(new Event("input", { bubbles: true }));
  };

  const emojiList = document.getElementById("hrVacancyEmojiList");
  if (emojiList) {
    emojiList.addEventListener("click", (event) => {
      const btn = event.target?.closest("button[data-emoji]");
      if (!btn) return;
      insertHrEmoji(btn.getAttribute("data-emoji") || "");
    });
  }

  const emojiTitle = document.querySelector(".hr-vacancy-emoji-title[data-i18n='hrQuickEmoji']");
  if (emojiTitle && String(emojiTitle.textContent || "").trim() === "hrQuickEmoji") {
    emojiTitle.textContent = t("hrQuickEmoji");
  }
...
function resetHrCreateVacancyForm() {
  ...
  hrEditingOpeningId = "";
  hrVacancyEmojiTarget = document.getElementById("hrVacancyRegulationInput") || document.getElementById("hrVacancyPositionInput") || null;
  const title = document.querySelector("#hrVacancyModal h3");
  if (title) title.textContent = t("hrCreateVacancy");
  const submitBtn = refs.hrCreateVacancyForm.querySelector("button[type='submit']");
  if (submitBtn) submitBtn.textContent = t("hrCreateBtn");
}
...
async function loadAttendanceFromApi() {
  try {
    ...
    // keep whatever was cached locally before; the CRM-activity heuristic
    // still covers managers without a real record.
  }
}
...
async function generateHrResumePdfDataUrl(row) {
  ...
  const position = safe(String(row.desiredPosition || row.position || "-"));
  const vacancyTitle = safe(String(row.vacancyTitle || row.position || "-"));
  ...
  const salaryNumber = salaryRaw.replace(/\D+/g, "");
  const salaryText = salaryNumber ? salaryNumber.replace(/\B(?=(\d{3})+(?!\d))/g, ".") : salaryRaw;
  const salary = salaryText ? `${salaryText} ${dict.currency[currencyCode] || currencyCode}` : "-";
  ...
  const drawWrapped = (text, x, startY, maxWidth, size, color, lineStep, font) => {
    const raw = safe(String(text || "-")).trim() || "-";
    const rows = raw.split(/\n+/).filter((lineText) => String(lineText || "").length);
    let line = "";
    let y = startY;
    const wrappedRows = rows.length ? rows : ["-"];
    wrappedRows.forEach((lineRaw) => {
      const words = String(lineRaw || "-").trim().split(/\s+/);
      line = "";
      words.forEach((word) => {
  ...
  line(dict.position, position || "-");
  if (vacancyTitle && vacancyTitle !== position) line(dict.vacancy, vacancyTitle);
...
async function loadHrVacancyOpenings() {
  ...
  if (typeof fetchVacancyOpeningsViaApi === "function") {
    ...
    if (Array.isArray(rows)) {
      state.db.vacancyOpenings = rows.map((row) => ({ ...row, regulation: ensureOpeningRegulation(row) }));
      localStorage.setItem(LS_DB, JSON.stringify(state.db));
...
function seedHrDemoOpeningsIfNeeded() {
  ...
  state.db.vacancyOpenings = [
    {
      id: uid("open"),
      position: "Yuk tashuvchi - o'rnatuvchi",
      regulation: "Ishga taklif qilinadi\n\nYuk tashuvchi - o'rnatuvchi\n(Gruzchik-ustanovshik)\n\nTalablar:\n- Erkak kishi\n- Jismonan baquvvat\n- Mebel sohasida tajriba\n\nIsh vaqti:\n- 09:00 dan 18:00 gacha\n\nOylik maosh:\n- 4 000 000 - 5 000 000 so'm\n\nTaklif qilamiz:\n- Qulay ish joyi va barqaror ish\n- Doimiy ish va rivojlanish imkoniyati\n\nBatafsil ma'lumot uchun:\n📞 95 885 22 33",
      createdAt: "2026-04-14T09:00:00+05:00",
      source: "vacancy_opening",
    },
    {
      id: uid("open"),
...
function renderHrKpis(managerStats) {
  ...
  const staffUsers = state.db.users || [];
  ...
  const staffTooltipRows = staffUsers
    .slice()
    .sort((a, b) => {
      const rank = staffRolePriority(a.role) - staffRolePriority(b.role);
      if (rank !== 0) return rank;
      return fullName(a).localeCompare(fullName(b), "uz", { sensitivity: "base" });
    })
    .map((user, idx) => `
      <div class="hr-kpi-tooltip-row">
        <span>${idx + 1}. ${escapeHtml(fullName(user) || user.login || "-")}</span>
        <em>${escapeHtml(staffRoleLabel(user.role))}</em>
      </div>
    `)
    .join("");
  const staffTooltip = staffTooltipRows || `<div class="hr-kpi-tooltip-row"><span>-</span><em>-</em></div>`;
  const cards = [
    {
...
function buildManagerActivityRank(managerStats, monthRows) {
  ...
  return (state.db.users || [])
    ...
    .map((manager) => {
      ...
      const totalClientActions = clients.filter((c) => c.managerId === managerId).length;
      ...
      const activityScore = Math.round(
        (loginCount * 2.6)
        + (monthClientActions * 3.4)
        + (totalClientActions * 1.1)
        + (monthSalesActions * 2.2)
        + (totalSales * 1.1)
        + (assignedByAdmin * 1.2)
        + (selfAdded * 1.4)
...
function buildAttendanceAnalytics(rangeOverride) {
  ...
  // Real check-in times (Telegram bot -> Google Sheets) take priority over
  // the CRM-activity heuristic below. Only managers without a real record
  // for a given day fall back to "first CRM action of the day".
  (state.db.attendance || []).forEach((row) => {
    if (!row.userId || !row.checkIn) return;
    const managerRow = byManager.get(row.userId);
    if (!managerRow) return;
    const ts = new Date(row.checkIn).getTime();
    if (!Number.isFinite(ts) || ts < range.fromTs || ts > range.toTs) return;
    const key = `${row.userId}|${toDayKey(ts)}`;
    dayFirstActivity.set(key, { ts, managerName: managerRow.name, isReal: true });
  });
...
function renderHrVacancies(vacancies) {
  ...
  if (positionFilter) {
    ...
    if (typeof enhanceSelectAsCustom === "function") enhanceSelectAsCustom(positionFilter);
  }
  ...
  refs.hrVacancyTbody.innerHTML = current
    .map((row, idx) => {
      const avatarRaw = row.avatarUrl || "";
      const avatar = isSafeUrl(avatarRaw) ? avatarRaw : defaultAvatar();
      ...
      const resumeHrefRaw = row.resumeUrl || row.resumeDataUrl || "";
      const resumeHref = isSafeUrl(resumeHrefRaw) ? resumeHrefRaw : "";
...
function renderHrOpenings(openings) {
  ...
  body.innerHTML = rows.map((row, idx) => `
    <tr>
      ...
      <td><span class="hr-opening-regulation-preview" title="${escapeHtml(ensureOpeningRegulation(row))}">${escapeHtml(openingRegulationPreview(row))}</span></td>
...
function openHrOpeningForEdit(id) {
  ...
  const regulation = ensureOpeningRegulation(row);
  if (refs.hrVacancyRegulationInput) refs.hrVacancyRegulationInput.value = regulation === "-" ? "" : regulation;
  const title = document.querySelector("#hrVacancyModal h3");
  if (title) title.textContent = t("hrCreateVacancy");
  const submitBtn = refs.hrCreateVacancyForm?.querySelector("button[type='submit']");
  if (submitBtn) submitBtn.textContent = t("save");
  hrVacancyEmojiTarget = refs.hrVacancyRegulationInput || refs.hrVacancyPositionInput || null;
  const dateInput = document.getElementById("hrVacancyDateInput");
  if (dateInput) {
