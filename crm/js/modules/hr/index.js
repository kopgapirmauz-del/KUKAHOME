...
function ensureOpeningRegulation(row) {
  ...
  const current = String(normalized.regulation || "").trim();
...
function renderHrOpenings(openings) {
  ...
  body.innerHTML = rows.map((row, idx) => `
    <tr>
      ...
      <td>${escapeHtml(ensureOpeningRegulation(row))}</td>
...
function openHrOpeningForEdit(id) {
  ...
  if (refs.hrVacancyRegulationInput) refs.hrVacancyRegulationInput.value = ensureOpeningRegulation(row) === "-" ? "" : ensureOpeningRegulation(row);
