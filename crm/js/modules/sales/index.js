...
async function generateSalesCheckPdfDataUrl(payload) {
  ...
  const midX = 900;
  const rightX = 1260;
  drawClamped(`Buyurtmaning umumiy summasi: ${payload.totalAmount}`, leftX, 730, 800);
  drawClamped(`Oldi to'lov miqdori: ${payload.prepayment}`, leftX, 770, 800);
  drawClamped(`To'lov usuli: ${payload.paymentMethod || ""}`, midX, 730, 300);
  drawClamped(`Yetkazib berish sanasi: ${payload.deliveryDate}`, rightX, 730, 490);
  drawClamped(`Yetkazib berish manzili: ${payload.deliveryAddress}`, rightX, 770, 490);

  const lineYs = [736, 776, 736, 736, 776];
  const lineStart = [430, 350, 1080, 1590, 1590];
  const lineEnd = [840, 840, 1220, 1760, 1760];
  for (let i = 0; i < lineYs.length; i += 1) {
    ctx.beginPath();
    ctx.moveTo(lineStart[i], lineYs[i]);
    ctx.lineTo(lineEnd[i], lineYs[i]);
    ctx.stroke();
  }
