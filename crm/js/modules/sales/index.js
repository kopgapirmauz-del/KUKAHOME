function renderSalesChecks() {
  ...
  refs.salesTbody.innerHTML = current.length
    ? current.map((row, idx) => {
      ...
      const canEditRow = canSalesAdmin();
      const canDeleteRow = typeof canDeleteSalesCheck === "function" ? canDeleteSalesCheck() : canSalesAdmin();
      const actionCell = (canEditRow || canDeleteRow)
        ? `<span class="chip-actions">${canEditRow ? `<button class="action-btn" data-sales-edit="${row.id}"><svg viewBox="0 0 24 24"><path d="m3 17.25 9.81-9.81 2.75 2.75L5.75 20H3v-2.75Zm14.71-8.04-2.92-2.92 1.42-1.42a1 1 0 0 1 1.42 0l1.5 1.5a1 1 0 0 1 0 1.42l-1.42 1.42Z"/></svg></button>` : ""}${canDeleteRow ? `<button class="action-btn" data-sales-delete="${row.id}"><svg viewBox="0 0 24 24"><path d="M6 7h12l-1 14H7L6 7Zm4-4h4l1 2h4v2H5V5h4l1-2Z"/></svg></button>` : ""}</span>`
        : "-";
...
function switchSettingsTab(tab) {
  ...
  const canShowChannels = Boolean(refs.settingsChannelsTab) && !refs.settingsChannelsTab.classList.contains("hidden");
  let normalizedTab = "profile";
  if (tab === "users" && canShowUsers) normalizedTab = "users";
  else if (tab === "channels" && canShowChannels) normalizedTab = "channels";
  state.settingsTab = normalizedTab;

  const pages = {
    profile: refs.settingsProfilePage,
    users: refs.settingsUsersPage,
    channels: refs.settingsChannelsPage,
  };
  Object.entries(pages).forEach(([key, el]) => {
    if (el) el.classList.toggle("hidden", key !== normalizedTab);
  });
  if (refs.settingsProfileTab) refs.settingsProfileTab.classList.toggle("active", normalizedTab === "profile");
  if (refs.settingsUsersTab) refs.settingsUsersTab.classList.toggle("active", normalizedTab === "users");
  if (refs.settingsChannelsTab) refs.settingsChannelsTab.classList.toggle("active", normalizedTab === "channels");

  if (normalizedTab === "channels" && typeof initSettingsChannelsUI === "function") {
    initSettingsChannelsUI();
  }
}
...
function renderSalesFilters() {
  ...
  if (typeof enhanceSelectAsCustom === "function") {
    enhanceSelectAsCustom(refs.salesStoreFilter);
    enhanceSelectAsCustom(refs.salesManagerFilter);
  }
}
...
function openSalesCheckModal(salesId) {
  ...
  if (salesId && !canSalesAdmin()) return;
...
async function generateSalesCheckPdfDataUrl(payload) {
  ...
  ctx.fillText(t("salesPdfTitle"), 900, 84);
  ctx.textAlign = "left";
  ctx.lineWidth = 2;
  ctx.beginPath();
  ctx.moveTo(24, 126);
  ...
  drawLabelValue(t("orderParty"), payload.customerName, 36, 150, 1000);
  drawLabelValue(t("customerName"), payload.customerName, 36, 190, 1000);
  drawLabelValue(t("customerPhone"), payload.customerPhone, 36, 230, 1000);
  drawLabelValue(t("date"), payload.orderDate, 36, 270, 1000);
  drawLabelValue(t("supplier"), "KUKA HOME", 1120, 150, 620);
  drawLabelValue(t("salesPdfSupplierStore"), payload.storeName, 1120, 190, 620);
  drawLabelValue(t("sellerNameLabel"), payload.sellerName, 1120, 230, 620);
  drawLabelValue(t("sellerPhone"), payload.sellerPhone, 1120, 270, 620);
  ...
  const headers = String(t("salesPdfHeaders") || "").split("|");
  ...
  const leftX = 36;
  const midX = 900;
  drawLabelValue(t("totalAmount"), payload.totalAmount, leftX, 730, 1100);
  drawLabelValue(t("prepayment"), payload.prepayment, leftX, 770, 1100);
  drawLabelValue(t("paymentMethod"), payload.paymentMethod || "", midX, 730, 500);
  drawLabelValue(t("deliveryDate"), payload.deliveryDate, leftX, 810, 1100);
  drawLabelValue(t("deliveryAddress"), payload.deliveryAddress, leftX, 850, 1600);

  ctx.font = "700 26px Montserrat, sans-serif";
  ctx.fillText(t("salesPdfInfoTitle"), 36, 910);

  const safeText = (value) => String(value || "__________");
  const infoLines = [
    [{ text: `${t("salesPdfQuestion1")} ` }, { text: safeText(payload.customerFloor), bold: true }, { text: ";" }],
    [{ text: `${t("salesPdfQuestion2")} ` }, { text: safeText(payload.elevatorInfo), bold: true }, { text: ";" }],
    [{ text: `${t("salesPdfQuestion3")} ` }, { text: safeText(payload.deliveryPaid), bold: true }, { text: ";" }],
    [{ text: `${t("salesPdfQuestion4")} ` }, { text: safeText(payload.doorFits), bold: true }, { text: ";" }],
    [{ text: `${t("salesPdfQuestion5")} ` }, { text: safeText(payload.agreesNoReturn), bold: true }, { text: ";" }],
    [{ text: `${t("salesPdfQuestion6")} ` }, { text: safeText(payload.warnedAboutIssue), bold: true }, { text: ";" }],
  ];
...
async function onSalesCheckSubmit(e) {
  e.preventDefault();
  if (!state.user) return;
  if (state.editingSalesCheckId && !canSalesAdmin()) return;
  if (!state.editingSalesCheckId && !canCreateSalesCheck()) return;
  try {
...
async function deleteSalesCheck(id) {
  if (typeof canDeleteSalesCheck === "function" ? !canDeleteSalesCheck() : !canSalesAdmin()) return;
