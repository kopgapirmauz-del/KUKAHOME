...
function openSalesCheckModal(salesId) {
  ...
  const syncSellerPhone = () => {
    ...
    if (!manager) return;
    refs.salesCheckForm.sellerPhone.value = manager.phone || refs.salesCheckForm.sellerPhone.value || "";
  };
...
async function generateSalesCheckPdfDataUrl(payload) {
  ...
  const infoLines = [
    [
      { text: "1. Mijoz uyining qavati: " },
      { text: safeText(payload.customerFloor), bold: true },
      { text: ", lift bormi va uning o'lchamlari qanday?: " },
      { text: safeText(payload.elevatorInfo), bold: true },
      { text: ", mijoz yetkazib berish uchun to'laydimi: " },
      { text: safeText(payload.deliveryPaid), bold: true },
      { text: ";" },
    ],
    [
      { text: "2. Kirish eshigining o'lchami katta o'lchamdagi mebelni olib kirishga javob beradimi?: " },
      { text: safeText(payload.doorFits), bold: true },
      { text: ";" },
    ],
    [
      { text: "3. Sotilgan mahsulotlar sifatida muammo bo'lmasa, qaytarib berish yoki almashtirish mumkin emas. Siz bunga rozimisiz? " },
      { text: safeText(payload.agreesNoReturn), bold: true },
      { text: ";" },
    ],
    [
      { text: "4. Agar siz muammosi bo'lgan mahsulot sotib olgan bo'lsangiz, sotuvchi sizga muammosi borligi haqida oldindan ogohlantirdimi? " },
      { text: safeText(payload.warnedAboutIssue), bold: true },
      { text: ";" },
    ],
  ];
  ...
  staticInfo.forEach((line, idx) => ctx.fillText(line, 36, 1045 + idx * 25));
