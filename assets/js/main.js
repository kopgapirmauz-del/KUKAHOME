(function () {
  ...
  function getCatalogProducts() {
    ...
    return fetch(getRootPrefix() + 'assets/data/products-manifest.json?v=20260706a', { cache: 'no-cache' })
      .then(function (res) {
  ...
  function initFurnitureGallery() {
    ...
    function renderFurnitureGrid(items, type, subtype, skipFinalize) {
      if (!grid) return;
      ...
      var list = type
        ...
        : getLatestItems(source, 16);
      grid.innerHTML = renderManifestItems(list);
    ...
    function renderFurnitureSubcategories(items, type, activeSubtype) {
      ...
      root.querySelectorAll('[data-subtype]').forEach(function (button) {
        button.addEventListener('click', function () {
          ...
          renderFurnitureGrid(items, type, subtype);
        });
      });
    }
    ...
    function bindFurnitureTypeFilters(items) {
      ...
      buttons.forEach(function (button) {
        ...
        button.addEventListener('click', function () {
          ...
          renderFurnitureGrid(items, type);
        });
      });
    ...
    if (grid || homeGrid) {
      Promise.resolve(getCatalogProducts())
        .then(function (items) {
          ...
          if (grid) {
            renderFurnitureGrid(items, '', '', true);
            bindFurnitureTypeFilters(items);
          }
          if (homeGrid) homeGrid.innerHTML = renderManifestItems(items.slice(-8).reverse());
        })
        .catch(function () {
