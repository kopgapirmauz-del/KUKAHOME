(function () {
  ...
  function getStoredLang() {
    try {
      return localStorage.getItem('kuka_lang') || 'uz';
    } catch (e) {
      return 'uz';
    }
  }
  ...
  function initVacancyPage() {
    ...
    var detailBody = document.getElementById('vacancy-detail-body');
    ...
    if (applyBtn) {
      applyBtn.addEventListener('click', function () {
        ...
        syncDesiredPosition(activeTitle);
    ...
    function renderVacancies(rows) {
      ...
      listRoot.querySelectorAll('[data-vacancy-id]').forEach(function (btn) {
        btn.addEventListener('click', function () {
          ...
          if (selectedVacancyInput) selectedVacancyInput.value = activeTitle;
          syncDesiredPosition(activeTitle);
          if (detailTitle) detailTitle.textContent = row.title;
          if (detailBody) detailBody.textContent = row.details || t('vacancy.detailsFallback');
    ...
    if (applyForm) {
      applyForm.addEventListener('submit', function (e) {
        ...
        if (!payload.fullName || !payload.vacancyTitle || !payload.birthDate || !payload.phone) {
          if (errorEl) errorEl.textContent = t('vacancy.requiredFields');
          return;
        }
        ...
        compressPhoto(photoFile)
          ...
          .then(function () {
            ...
            if (desiredPositionInput && selectedVacancyInput) {
              syncDesiredPosition(selectedVacancyInput.value || '');
            }
          })
          .catch(function () {
