(function () {
  ...
  function initVacancyPage() {
    ...
    function syncDesiredPosition(title) {
      ...
      var map = [
        { test: 'sotuv', value: 'Sotuv menedjeri' },
        { test: 'kassir', value: 'Kassir' },
        { test: 'ombor', value: 'Omborchi' },
        { test: 'marketing', value: 'Marketing mutaxassisi' },
        { test: 'smm', value: 'SMM menedjer' },
        { test: 'hr', value: 'HR' },
        { test: 'target', value: 'Targetolog' },
        { test: 'call', value: 'Call markazi' }
      ];
    ...
    if (addJobRowBtn && jobRowsRoot) {
      addJobRowBtn.addEventListener('click', function () {
        ...
        row.innerHTML = ''
          + '<input type="text" name="jobCompany[]" placeholder="Qayerda ishlagan">'
          + '<input type="text" name="jobRole[]" placeholder="Lavozimi">'
          + '<input type="text" name="jobYears[]" placeholder="Nechi yil ishlagan">'
          + '<button type="button" class="vacancy-job-remove" data-job-remove aria-label="Qatorni o\'chirish">'
    ...
    function renderVacancies(rows) {
      ...
      listRoot.querySelectorAll('[data-vacancy-id]').forEach(function (btn) {
        btn.addEventListener('click', function () {
          ...
          if (detailBody) detailBody.textContent = row.details || 'Vakansiya bo\'yicha batafsil ma\'lumotni HR bo\'limidan olishingiz mumkin.';
    ...
    if (applyForm) {
      applyForm.addEventListener('submit', function (e) {
        ...
        var expectedSalary = (applyForm.querySelector('input[name="expectedSalary"]') || {}).value || '';
        ...
        var desiredPosition = (applyForm.querySelector('select[name="desiredPosition"]') || {}).value || '';
        var vacancyTitle = desiredPosition || ((selectedVacancyInput && selectedVacancyInput.value) ? selectedVacancyInput.value : activeTitle);
        var langValues = [];
        ...
        if (!payload.fullName || !payload.vacancyTitle) return;
        if (submitBtn) submitBtn.disabled = true;
        if (successEl) successEl.textContent = '';
        if (errorEl) errorEl.textContent = '';

        submitVacancyApplication(payload)
          .then(function () {
            if (successEl) successEl.textContent = 'Arizangiz yuborildi. HR bo\'limi siz bilan bog\'lanadi.';
            applyForm.reset();
            if (jobRowsRoot) {
          ...
          .catch(function () {
            if (errorEl) errorEl.textContent = 'Xatolik yuz berdi. Iltimos, qayta urinib ko\'ring.';
          })
          .finally(function () {
