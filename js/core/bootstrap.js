...
function bindEvents() {
  ...
  document.addEventListener("click", (e) => {
    ...
    if (!e.target.closest(".client-contact-wrap")) {
      document.querySelectorAll(".client-contact-wrap.open").forEach((el) => el.classList.remove("open"));
    }
  });
...
function renderNotifications() {
  ...
  refs.notifList.querySelectorAll(".notif-item[data-notif-open]").forEach((item) => {
    item.addEventListener("click", async () => {
      ...
      refs.notifMenu.classList.add("hidden");
      switchPage("clients");
      const candidateRows = getFilteredClients();
      let targetClient = candidateRows.find((c) => String(c.id || "") === directClientId);
      if (!targetClient) {
        const allRows = state.db.clients || [];
        targetClient = allRows.find((c) => String(c.id || "") === directClientId) || null;
      }
      if (!targetClient) state.filters.search = String(item.dataset.notifContact || "").trim();
      state.pageIndex = 1;
      renderFilters();
      renderTableWithLoading();
      if (targetClient) {
        state.filters.search = String(targetClient.contact || "");
        renderFilters();
        renderTableWithLoading();
      }
    });
