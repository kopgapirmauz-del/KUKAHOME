...
async function loadHrVacancies() {
  ...
  if (typeof fetchVacanciesViaApi === "function") {
    ...
    if (Array.isArray(rows)) {
      ...
      ensureHrVacancyResumes(rows)
        .then(() => renderHrVacancies(state.db.vacancies || []))
        .catch(() => {});
      return rows;
    }
  }
  ...
  ensureHrVacancyResumes(state.db.vacancies)
    .then(() => renderHrVacancies(state.db.vacancies || []))
    .catch(() => {});
...
function seedHrDemoVacanciesIfNeeded() {
  ...
  if (state.db.meta.hrVacancyDemoSeeded) return;
  ...
  const demo = [
    ["Anvar Sattorov", "Sotuv menedjeri"], ["Charos Ramonberganova", "HR"], ["Dilnoza Chen", "Kassir"],
    ["Durdona Imomberdiyeva", "Marketing mutaxassisi"], ["Mulisa Rahmanova", "Sotuv menedjeri"], ["Suleyman Musayev", "Skladchi"],
    ["Xurshida Sattorova", "SMM menejer"], ["Jahongir Azizov", "Call markazi"], ["Nigora Jalilova", "Targetolog"],
    ["Miraziz Tursunov", "Omborchi"], ["Kamola Ergasheva", "Kassir"], ["Temur Raxmatov", "HR"],
  ];
  state.db.vacancies = demo.map((row, idx) => ({
    id: uid("vac_demo"),
    fullName: row[0],
...
async function generateHrResumePdfDataUrl(row) {
  ...
  page.drawRectangle({ x: 0, y: height - 130, width, height: 130, color: PDF.rgb(0.94, 0.95, 0.97) });
  page.drawText(title, { x: 36, y: height - 68, size: 24, color: PDF.rgb(0.12, 0.12, 0.12) });
  page.drawText(position, { x: 36, y: height - 95, size: 14, color: PDF.rgb(0.55, 0.2, 0.2) });

  let y = height - 170;
  const line = (label, value) => {
    page.drawText(label, { x: 36, y, size: 11, color: PDF.rgb(0.45, 0.45, 0.45) });
    page.drawText(String(value || "-"), { x: 190, y, size: 12, color: PDF.rgb(0.1, 0.1, 0.1) });
    y -= 24;
  };
  line("Telefon", phone);
  line("Ariza manbasi", String(row.source || "website"));
  line("Ariza sanasi", created);
  line("Holat", String(row.status || "new"));

  y -= 10;
  page.drawText("Qisqacha ma'lumot", { x: 36, y, size: 14, color: PDF.rgb(0.15, 0.15, 0.15) });
  y -= 24;
  const summary = safe(String(row.note || `${title} ${position} lavozimi uchun sayt orqali ariza topshirgan.`));
  page.drawText(summary.slice(0, 180), { x: 36, y, size: 11, color: PDF.rgb(0.2, 0.2, 0.2) });
...
function renderHrVacancies(vacancies) {
  ...
  if (!filtered.length) {
    refs.hrVacancyTbody.innerHTML = `<tr><td colspan="6">${escapeHtml(t("hrNoVacancies"))}</td></tr>`;
  ...
  refs.hrVacancyTbody.innerHTML = current
    .map((row, idx) => {
      ...
      const resumeCell = resumeUrl
        ? `<a class="action-btn sales-upload-btn" href="${escapeHtml(resumeUrl)}" target="_blank" rel="noopener noreferrer" title="${escapeHtml(t("hrResumeFile"))}"><svg viewBox="0 0 24 24"><path d="M5 20h14v-2H5v2Zm7-16v10.16l-3.08-3.08-1.42 1.42L12 18l5.5-5.5-1.42-1.42L13 14.16V4h-2Z"/></svg></a>`
...
function exportHrVacanciesExcel() {
  ...
  const headers = [t("number"), t("hrCandidate"), t("hrPosition"), t("hrResumeFile")];
  const body = rows.map((row, idx) => [idx + 1, row.fullName || "", row.position || "", row.resumeUrl || ""]);
