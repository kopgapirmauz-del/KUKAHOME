...
import {
  buildFacebookPageAuthorizationUrl,
...
export async function onRequestPost(context) {
  ...
  try {
    ...
    const state = await createMetaOAuthState(env, session, config.redirectUri, "facebook");
    return Response.json({
      success: true,
      authorization_url: buildFacebookPageAuthorizationUrl(config, state),
      expires_in: 600,
    }, {
      headers: {
    ...
    const code = String(error?.message || "meta_facebook_oauth_start_failed");
    const known = ["meta_oauth_not_configured", "invalid_meta_oauth_redirect"].includes(code);
    return Response.json({
      success: false,
      error: known ? code : "meta_facebook_oauth_start_failed",
    }, {
